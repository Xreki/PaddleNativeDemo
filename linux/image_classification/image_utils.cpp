...
bool ImageReader::operator()(const std::string& image_path,
  ...
  if (image_width != width || image_height != height || image_channel != channel) {
  ...
  if (channel == 3) {
      ...
      for (size_t c = 0; c < channel; c++) {
        for (size_t y = 0; y < height; y++) {
          for (size_t x = 0; x < width; x++) {
            data[index] =
                static_cast<float>(image.at<cv::Vec3b>(y, x)[c]);
            index++;
          }
        }
      }
    ...
    for (size_t y = 0; y < height; y++) {
      for (size_t x = 0; x < width; x++) {
        data[index] = static_cast<float>(image.at<unsigned char>(y, x));
        index++;
      }
    }
  }
